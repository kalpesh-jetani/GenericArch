...
#
# CLAUDE.md §2.2: the working tree is left alone and the user commits when they
...
CMD="$DIR/09-commit.sh"
{
  ...
  printf '# Task: %s/%s\n#\n' "$PROJECT" "$TASK_ID"
  printf '# The pipeline did NOT run this (CLAUDE.md §2.2). Review, then:\n'
  printf '#   bash %s\n#\n' "$CMD"
...
printf '\n'
dim "CLAUDE.md §2.2 — this tooling never commits or pushes. The working tree is"
