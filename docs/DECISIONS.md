...
| Tool profiles at adopt time | **Scaffolded, never copied.** A target gets `.claude/tools/CLAUDE.md` verbatim and a header-only `LEDGER.tsv`; no `<tool>.md`, no `Scripts/Generated/`, and the generated spans in `MAP.tsv`, `INDEX.md` and the registry note are blanked between their markers | `.claude/tools` in `BASE` shipped this repo's own profile — the file, a ledger row marked *active* and *verified*, and rows in all four index surfaces — so a consumer held a verified profile no session of theirs ever observed. Same shape as the *Open* question about the nine notes arriving with this base's example rows, and the same answer: prose and rules travel, observations do not | [TOOL-PROFILES.md](TOOL-PROFILES.md), [`Scripts/adopt.sh`](../Scripts/adopt.sh) |
