...
| Why a rule was chosen, or what it replaced | [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md) |
| History — "X was removed", "this used to be Y" | [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md) |
...
| `<component>/CLAUDE.md` | rules that bind only inside one component (§2.6) | **only when that directory is touched** |
...
*projection* of the six above — generated from `CLAUDE.md` §2 and [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md) by
...
[OPENSPEC.md](/docs/reference/OPENSPEC.md)(/docs/reference/OPENSPEC.md) exists and why `Scripts/check.sh` warns when the projection goes stale.

Deciding where something goes:

- A rule true only inside one component → that component's `CLAUDE.md` (§2.6). Never the root
...
appended by `./Scripts/ga-log.sh` and read by nobody but the user. §2.7 sends narration to the
commit message, but §2.2 forbids committing — so without this home the reasoning either
lands in a reusable file where it rots, or is lost. **The log is never read to decide
anything**; that is [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md), which is normative. One records the
...
**A skill that fires wrongly is a description bug.** CLAUDE.md §2.3 requires naming the skill
...
- **Nothing already recorded in `CLAUDE.md`, this file or
  [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md) belongs in memory.** Rules live at the level that ships. A memory that
...
per-module docs were (/docs/decisions/DECISIONS.md).
...
## Component reference doc — optional, and deliberately so
...
loop this base removed when it retired its own per-module docs (/docs/decisions/DECISIONS.md).

The shape a product's layers take is the active profile's.
...
```markdown
✅ `src/nav/Router.ext:12` — the case declaration
✅ click homeFeed "…/FeedView.ext" "Paged feed — the paged-content state"
❌ `src/nav/Router.ext`
```
...
```markdown
✅ 2. The handler exposes its state —
      `src/features/auth/login.ext` — note how the failure case keeps the
...
([PATTERN-SEARCH.md](/docs/patterns/PATTERN-SEARCH.md)(/docs/patterns/PATTERN-SEARCH.md)). One `grep` returns the row, the row answers the question,
...
- **A note is maintained by inserting and deleting rows** — a row appears when the thing appears and
  goes when the thing goes, in the same change (CLAUDE.md §3). `/sync-app-notes` is the single
