...
valid JSON and parseable by one `awk`, because not all systems have `jq` and an installer that
...
installed file — `/project-init`, `/sync-app-notes` — therefore turns that file into a
...
```json
{
  ...
  "files": [
    ...
    {"path": ".lintrc", "action": "skipped", "sha256": "c07f33...", "installed_at": "2026-03-02T11:20:44Z", "backup": null, "original_sha256": null, "version": "v0.6.0"},
...
Reading that back: `MAP.tsv` and `check.sh` are ours and go on uninstall. `.lintrc` was
...
**What the fallback can and cannot reach, as of v0.5.0.** Ownership is proven by hashing against the
...
and `:remote` marks, `SCRIPTS.tsv` is pruned, and `notes/` and `DECISIONS.md` are
...
`docs/DECISIONS.md`, `genericarch.installation.md` — have no shipped blob to compare
