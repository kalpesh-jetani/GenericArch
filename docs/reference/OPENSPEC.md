...
change it — and [STRUCTURE.md](/docs/reference/STRUCTURE.md)(/docs/reference/STRUCTURE.md) is explicit that when a thing is stated twice, the copy
is what rots.
...
|---|---|---|
| **GenericArch** | The rules: `CLAUDE.md` §2, [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md), the active profile's rules, and the `.claude/notes/` inventories | us |
| **The product repo** | Its own reconciled `CLAUDE.md` and its own answers — see [adoption/ADOPTION.md](/docs/adoption/ADOPTION.md)(/docs/adoption/ADOPTION.md) Path A | the product |
...
does not travel and [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md) arrives empty, so in an adopted repo the
...
| specs | `rules.specs` | the active profile's data-and-state rules and its localization rules |
| design | `rules.design` | the active profile's structural rules — module boundaries, dependency wrapping, injection, platform floors, extraction tests |
| tasks | `rules.tasks` | Close every task list with a GenericArch memory group — items and their verification taken from the active profile's definition-of-done |
| apply | `operations.apply.guidance` | §2.2 never commit · §2.8 build freely, never run |
...
- A line in `CLAUDE.md` §3 is the only unconditional path, because that file loads every session.
  **It is present in this base** — §3, directly under the memory paragraph. In a consumer repo it
  may not be: `CLAUDE.md` does not travel, and adding it there is a §4 edit that repo has to
...
**Their verify step cannot be injected either.** The active profile's definition-of-done and `/verify` remain the
...
| Row | Means |
|---|---|
| `IN-FLIGHT` | An active change. No scaffolder must overwrite it |
...
CLAUDE.md §2.2 is not in play and this needs no exception to it. Fixing the parser afterwards is
...
Run it after editing `CLAUDE.md` §2 or [DECISIONS.md](/docs/decisions/DECISIONS.md)(/docs/decisions/DECISIONS.md), **and after every upstream
