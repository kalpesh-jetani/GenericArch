...
- **Driven by:** a stack profile's scaffolder, or by hand.
...
| `never:<reason>` | Claude must not run it. Nothing carries it today — `check.sh` became `call` once §2.8 made compiling the way a change gets validated |
...
These scripts run on any POSIX host. They stick to portable constructs — no associative arrays, no
`mapfile`, no `${var^^}` — and reach for a hash tool (`shasum` or `sha256sum`) through
`ga-lifecycle.sh` rather than assuming one.

One portability wrinkle is worth naming: BSD awk counts `length()` in bytes where GNU counts
characters, so a line-length rule can report different numbers on each. That is why `claude-lint.sh`
measures with its own `dlen()` — an em-dash would otherwise inflate a line by two, which produced a
page of false positives on these docs.

## Build tools

The pipeline never builds (§2.8). What it does instead:

- The project registry (`projects.tsv`) records each project's root, its `CLAUDE.md` path, its test
  command and its source glob — declared in the row, not detected, so nothing here shells out to a
  build tool.
- Phase 7 syntax-checks the snippets in a CLAUDE.md across whatever languages they are written in
  (bash, python, json, yaml, js, ts, go, rust, …), parsing each with that language's own parser
  when one is available and skipping it with a note when it is not. It never compiles or runs a
  project.
- Findings print an editor-open hint, so a reported line opens where it lives instead of being
  hunted for. Printed, never run — a script that seized the editor would be a surprise.

## Two gates that never open on their own

| Gate | Rule | What the script does |
|---|---|---|
| **Phase 5 writes** | [STRUCTURE.md](/docs/reference/STRUCTURE.md)(/docs/reference/STRUCTURE.md) — CLAUDE.md is approval-gated | Refuses without `--approve`; prints what it would do and exits 4 |
| **Phase 9 commits** | CLAUDE.md §2.2 — never commit or push | Never runs git. Writes `09-commit.sh` for you to run |

Phase 7 is the third: CLAUDE.md §2.8 forbids this tooling from invoking a compiler, so it extracts
...
```bash
# register a project by adding a row to projects.tsv (columns below); list them with:
...
path, its test command and the source glob phase 7 uses. These are declared in the row, not
detected — so nothing here shells out to a build tool, and a wrong value is visible rather than
silent.
