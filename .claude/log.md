...
- **Decided:** ga_need_val now guards every value-taking flag in ga-remove.sh, ga-reseal.sh, ga-project-setup.sh and ga-step.sh; ga-handoff.sh carries a local copy
  **Why:** Finishing the class the tool-profile walk exposed. Two earlier claims were wrong and are corrected here: ga-sync-scan.sh was ALREADY guarded (shift 2 || ga_die), and ga-roundtrip.sh never hung — it has no arg loop and my 6s cutoff misread its slow round-trip suite as a spin. Conversely ga-step.sh DID hang and was missed, because 'cd "" ' SUCCEEDS in bash, so its '|| ga_die no such directory' never fired and the failed shift 2 spun. 'shift 2 || true' is not a guard either: it silences the error but leaves $1 unchanged, so the loop still never ends.
  **How:** ga_need_val at 12 sites across the four library-sourcing scripts. ga-handoff.sh sources nothing by design — it is what runs when other scripts are broken — so it got a local need_val in its own style rather than a new dependency. Verified by sweeping all 61 value-taking flags in every ga-* script with an arg loop: 0 hang. GA_LIB_VERSION not bumped — no function was added this time.
  *(2026-09-15T09:33:13Z)*
